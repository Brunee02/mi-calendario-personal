...
function openTask(id, date) {
  ...
  $("tomorrowBtn").onclick = () => {
    ...
    const tomorrow = addDays(oldDate, 1);

    // Mover la ocurrencia actual a mañana.
    // Para pendientes únicos, se mueve el registro original.
    if (base.repeat === "once" || !base.repeat) {
      // Una vez = mover el MISMO registro. Nunca crear una serie ni una copia.
      base.repeat = "once";
      base.date = tomorrow;
      base.status = "pending";
      base.series = null;
      // El estado pertenece a la ocurrencia anterior; lo eliminamos para que
      // el pendiente pueda aparecer limpio y seleccionable mañana.
      localStorage.removeItem("status_" + base.id + "_" + oldDate);
      localStorage.removeItem("status_" + t.id + "_" + oldDate);
      save(all);
    } else {
      // Para repetitivos, no modificamos la serie original:
      // creamos una ocurrencia única para mañana y dejamos intacta la serie.
    ...
    // Sincronizar las dos referencias de fecha/filtro que usa la app.
    // IMPORTANT: la app tiene dos referencias de estado (las originales
    // y las usadas por la capa final). Sincronizamos ambas para que
    // el pendiente no parezca desaparecer en iPhone/Safari.
    selectedDate = tomorrow;
    currentFilter = "today";
    window.selectedDate = tomorrow;
    window.currentFilter = "today";
    calendarMonth = dateFromString(tomorrow);
    browseMonth = dateFromString(tomorrow);
    window.calendarMonth = new Date(
      dateFromString(tomorrow).getFullYear(),
      dateFromString(tomorrow).getMonth(),
      1
    );

    $("taskDetailDialog").close();

    // Leer nuevamente desde localStorage y reconstruir la vista.
    // Esto evita que el render conserve el pendiente de la fecha anterior.
...
/* =========================
   FINANZAS - PROTOTIPO LOCAL
   ========================= */
(function(){
  const FIN_KEY = "mi_calendario_finanzas_v1";
  const GOAL_KEY = "mi_calendario_finance_goals_v1";
  let financeMonth = new Date();
  const money = n => `S/ ${Number(n||0).toLocaleString("es-PE",{minimumFractionDigits:2,maximumFractionDigits:2})}`;
  const fday = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
  const fload = () => { try{return JSON.parse(localStorage.getItem(FIN_KEY)||"[]")}catch{return[]} };
  const fsave = a => localStorage.setItem(FIN_KEY,JSON.stringify(a));
  const gload = () => { try{return JSON.parse(localStorage.getItem(GOAL_KEY)||"[]")}catch{return[]} };
  const gsave = a => localStorage.setItem(GOAL_KEY,JSON.stringify(a));
  const monthKey = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
  const inMonth = (x,d) => x.date && x.date.slice(0,7)===monthKey(d);
  const financeData = () => fload().filter(x=>inMonth(x,financeMonth));
  ...
  function renderFinance(){
    ...
    qf('financeMonth').textContent=financeMonth.toLocaleDateString('es-PE',{month:'long',year:'numeric'});
    qf('financeBalance').textContent=money(balance);
    qf('financeIncome').textContent=money(income);
    qf('financeExpense').textContent=money(expense);
    qf('financeSavings').textContent=`${savingRate.toFixed(0)}%`;
    qf('financeBalanceMeta').textContent=balance>=0?'Disponible después de tus gastos':'Has gastado más de lo ingresado';
    const cats={};items.filter(x=>x.type==='expense').forEach(x=>cats[x.category]=(cats[x.category]||0)+x.amount);
    const entries=Object.entries(cats).sort((a,b)=>b[1]-a[1]);
    qf('financeTopCategory').textContent=entries[0]?`${entries[0][0]} · ${money(entries[0][1])}`:'';
    qf('financeCategories').innerHTML=entries.length?entries.map(([name,val])=>{const pct=expense?Math.min(100,val/expense*100):0;return `<div class="finance-bar-row"><div><span>${esc(name)}</span><strong>${money(val)}</strong></div><div class="finance-bar"><i style="width:${pct}%"></i></div></div>`}).join(''):'<div class="finance-empty">Aún no registras gastos este mes.</div>';
    const goals=gload();
    qf('financeGoals').innerHTML=goals.length?goals.map(g=>{const pct=Math.min(100,(g.saved/g.target)*100);return `<div class="goal-row"><div class="goal-top"><strong>${esc(g.name)}</strong><span>${money(g.saved)} / ${money(g.target)}</span></div><div class="finance-bar"><i style="width:${pct}%"></i></div><div class="goal-actions"><small>${pct.toFixed(0)}% completado</small><button type="button" data-goal-add="${g.id}">＋ Aportar</button><button type="button" data-goal-delete="${g.id}">Eliminar</button></div></div>`}).join(''):'<div class="finance-empty">Crea una meta para empezar a ahorrar.</div>';
    qf('financeAdvice').innerHTML=buildAdvice(income,expense,balance,entries);
    const recent=fload().sort((a,b)=>b.date.localeCompare(a.date)||b.createdAt-a.createdAt).slice(0,8);
    qf('financeTransactions').innerHTML=recent.length?recent.map(x=>`<div class="finance-tx"><div><strong>${x.type==='income'?'＋':'−'} ${esc(x.description)}</strong><small>${esc(x.date)} · ${esc(x.category)}</small></div><b class="${x.type}">${x.type==='income'?'+':'−'}${money(x.amount)}</b><button type="button" data-fin-delete="${x.id}">×</button></div>`).join(''):'<div class="finance-empty">Tus movimientos aparecerán aquí.</div>';
    qf('financeCategories').querySelectorAll('.finance-bar-row').forEach(()=>{});
    qf('financeGoals').querySelectorAll('[data-goal-add]').forEach(b=>b.onclick=()=>{const goals=gload(),g=goals.find(x=>x.id===b.dataset.goalAdd);if(!g)return;const amount=prompt(`¿Cuánto quieres aportar a ${g.name}?`);const n=Number(amount);if(n>0){g.saved=Math.min(g.target,g.saved+n);gsave(goals);renderFinance();}});
    qf('financeGoals').querySelectorAll('[data-goal-delete]').forEach(b=>b.onclick=()=>{if(confirm('¿Eliminar esta meta?')){gsave(gload().filter(g=>g.id!==b.dataset.goalDelete));renderFinance();}});
    qf('financeTransactions').querySelectorAll('[data-fin-delete]').forEach(b=>b.onclick=()=>{if(confirm('¿Eliminar este movimiento?')){fsave(fload().filter(x=>x.id!==b.dataset.finDelete));renderFinance();}});
  }
  function buildAdvice(income,expense,balance,entries){
    if(!income&&!expense)return '<div class="advice-item">Empieza registrando tus ingresos y gastos. Con unos pocos movimientos podremos detectar patrones y darte recomendaciones.</div>';
    const out=[];
    if(balance<0)out.push('Este mes tus gastos superan tus ingresos. Prioriza los gastos esenciales y revisa los gastos variables antes de asumir nuevos compromisos.');
    else if(income>0 && balance/income<.1)out.push('Tu margen disponible está por debajo del 10%. Si quieres ahorrar, prueba una meta pequeña y reduce primero una categoría variable.');
    else if(income>0)out.push(`Tu margen actual es de ${money(balance)}. Podrías separar una parte como ahorro antes de gastar el resto.`);
    if(entries[0])out.push(`Tu categoría con mayor gasto es ${entries[0][0]} (${money(entries[0][1])}). Revisarla primero tendría mayor impacto en tu presupuesto.`);
    if(entries.length>=3)out.push('Tienes gastos repartidos en varias categorías. Un límite mensual por categoría puede hacer más fácil controlar el presupuesto.');
    return out.map(x=>`<div class="advice-item">💡 ${esc(x)}</div>`).join('');
  }
  function openEntry(type){
    qf('financeEntryEyebrow').textContent=type==='income'?'INGRESO':'GASTO';qf('financeEntryTitle').textContent=type==='income'?'Registrar ingreso':'Registrar gasto';qf('financeEntryForm').dataset.type=type;qf('financeEntryForm').reset();qf('financeDate').value=fday(new Date());qf('financeEntryDialog').showModal();
  }
  qf('financeBtn').onclick=()=>{financeMonth=new Date();qf('financeDialog').showModal();renderFinance();};
  qf('closeFinance').onclick=()=>qf('financeDialog').close();
  qf('financePrev').onclick=()=>{financeMonth=new Date(financeMonth.getFullYear(),financeMonth.getMonth()-1,1);renderFinance();};
  qf('financeNext').onclick=()=>{financeMonth=new Date(financeMonth.getFullYear(),financeMonth.getMonth()+1,1);renderFinance();};
  qf('addExpenseBtn').onclick=()=>openEntry('expense');
  qf('addIncomeBtn').onclick=()=>openEntry('income');
  qf('closeFinanceEntry').onclick=()=>qf('financeEntryDialog').close();
  qf('financeEntryForm').onsubmit=e=>{e.preventDefault();const a=fload();a.push({id:crypto.randomUUID(),type:e.currentTarget.dataset.type,amount:Number(qf('financeAmount').value),description:qf('financeDescription').value.trim(),category:qf('financeCategory').value.trim()||'Otros',date:qf('financeDate').value,createdAt:Date.now()});fsave(a);qf('financeEntryDialog').close();renderFinance();};
  qf('addGoalBtn').onclick=()=>{qf('goalForm').reset();qf('goalSaved').value='0';qf('goalDialog').showModal();};
  qf('closeGoal').onclick=()=>qf('goalDialog').close();
  qf('goalForm').onsubmit=e=>{e.preventDefault();const a=gload();a.push({id:crypto.randomUUID(),name:qf('goalName').value.trim(),target:Number(qf('goalTarget').value),saved:Number(qf('goalSaved').value||0)});gsave(a);qf('goalDialog').close();renderFinance();};
})();

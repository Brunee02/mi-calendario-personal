...
(function(){
  ...
  function updateNotificationUI(){const t=q('notificationTitle'),s=q('notificationStatus');if(!t||!s)return;if(!('Notification'in window)){t.textContent='Notificaciones no disponibles';s.textContent='Este navegador no ofrece esta función.';}else if(Notification.permission==='granted'){t.textContent='Notificaciones activadas';s.textContent='Avisos mientras esta página esté abierta.';}else if(Notification.permission==='denied'){t.textContent='Notificaciones bloqueadas';s.textContent='Permítelas desde la configuración del navegador.';}else{t.textContent='Activar notificaciones';s.textContent='Avisos del navegador para tus pendientes.';}}
  function requestNotifications(){if(!('Notification'in window)){alert('Este navegador no permite notificaciones.');return;}Notification.requestPermission().then(()=>{updateNotificationUI();});}
